    ...
    *,
    ...
    try:
        from rbac.subfeature_access import (
            notifications_can_history,
            notifications_can_send,
            notifications_can_templates,
        )
    except Exception:

        def notifications_can_send(user=None) -> bool:  # type: ignore[misc]
            return True

        def notifications_can_history(user=None) -> bool:  # type: ignore[misc]
            return True

        def notifications_can_templates(user=None) -> bool:  # type: ignore[misc]
            return True

    labels: list[str] = []
    if notifications_can_send():
        labels.append("Send Notification")
    if notifications_can_history():
        labels.append("History")
    if notifications_can_templates():
        labels.append("Templates")
    if not labels:
        st.warning("You do not have permission for any Notifications areas for this role.")
        return

    _tabs = st.tabs(labels)

    def _ix(name: str) -> int | None:
        return labels.index(name) if name in labels else None

    _send_i = _ix("Send Notification")
    _hist_i = _ix("History")
    _tpl_i = _ix("Templates")
