...
def _credit_unapplied_funds(
    ...
    Guardrail: overpayment credits must be backed by loan_repayment_allocation.unallocated.
    """
    ...
    with conn.cursor() as cur:
        ...
        if isinstance(row, tuple):
            unallocated_raw = row[0]
        else:
            unallocated_raw = row.get("unallocated", 0)
        unallocated = float(as_10dp(unallocated_raw or 0))
