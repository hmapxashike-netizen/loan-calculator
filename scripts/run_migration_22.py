"""
Run schema migration 22_unapplied_single_table_and_lineage.sql.
Consolidates unapplied_funds + unapplied_funds_ledger into one ledger-style table.
Adds lineage (source_repayment_id) for reversal cascade.

From project root:
  python scripts/run_migration_22.py
"""
import os
import sys
...
def main():
    ...
    sql_path = os.path.join(schema_dir, "22_unapplied_single_table_and_lineage.sql")
    ...
    try:
        ...
        with conn.cursor() as cur:
            cur.execute(sql)
        conn.close()
        print("Migration 22 (unapplied single table and lineage) applied successfully.")
